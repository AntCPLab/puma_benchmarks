...
tokenizer = AutoTokenizer.from_pretrained("marciovbarbosa/t5-small-finetuned-de-to-en")
pretrained_model = FlaxT5ForConditionalGeneration.from_pretrained("marciovbarbosa/t5-small-finetuned-de-to-en", from_pt=True)
...
def tokenize_and_preprocess(example):
    ...
    inputs = tokenizer(source_text, return_tensors="jax", max_length=32, truncation=True, padding=True)
    return inputs
...
reference = tokenized_dataset[:NUM]["translation"]["en"]

inputs_ids = tokenized_dataset["input_ids"]

def translation_generation(input_ids, params):
    pretrained_model.params = params
    translation_ids = pretrained_model.generate(input_ids).sequences
...
def run_on_cpu():
    ...
    for i in range(NUM):
        input_ids = jnp.array(inputs_ids[i])
        generated_ids = pretrained_model.generate(input_ids).sequences
        generated_text = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
        generated_translation.extend(generated_text)
...
def run_on_griffin():
    ...
    for i in range(NUM):
        ...
        with hack_softmax_context("hack jax softmax", enabled=True), hack_gelu_context("hijack jax gelu", enabled=True):
            generated_ids = ppd.device("SPU")(
                translation_generation, copts=copts,
            )(input_ids, params)
            generated_ids = ppd.get(generated_ids)
            generated_text = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
            generated_translation.extend(generated_text)
...
if __name__ == '__main__':
    ...
    rouge = Rouge()
    print('\nmarciovbarbosa/t5-small-finetuned-de-to-en, Translation-de-en, WMT16, Num=100, Max New Token 128\n')
    print('\n------\nRun on CPU')
    generated_translation_cpu = run_on_cpu()
    scores = rouge.get_scores(generated_translation_cpu, reference, avg=True)
    ...
    print('\n------\nRun on SPU')
    generated_translation_spu = run_on_spu()
    scores = rouge.get_scores(generated_translation_spu, reference, avg=True)
    ...
    print('\n------\nRun on Griffin')
    generated_translation_griffin = run_on_griffin()
    scores = rouge.get_scores(generated_translation_griffin, reference, avg=True)
    # Print ROUGE scores for Griffin
